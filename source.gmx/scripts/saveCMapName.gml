...
with all {
    ...
    var yy = y
    file_text_write_string(f,'-'+padStringLeft(intToBaseXString(y+128,32),2,'0'))
    with all {
        ...
        if saveid != -1 {
            var idstring = intToBaseXString(saveid,64)
            if string_length(idstring) > 1 {
                file_text_write_string(f,"*" + string(string_length(idstring) - 1))
            }
            file_text_write_string(f,intToBaseXString(saveid,64)
                +padStringLeft(intToBaseXString(x+128,32),2,'0'))
            saved = true
        }
        else {
